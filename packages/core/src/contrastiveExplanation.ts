...
import type { ExecutedItem } from './pipeline-types';
import type { ScoreExplanationFactor } from './scoreRecovery';
...
export function generateContrastiveReport(
  ...
  const comparisons: ContrastivePeerComparison[] = selectedPeers.map((peer) => {
    ...
    const targetFactors = new Map(targetItem.score.explanation.map((f: ScoreExplanationFactor) => [f.factor, f]));
    const peerFactors = new Map(peer.score.explanation.map((f: ScoreExplanationFactor) => [f.factor, f]));
