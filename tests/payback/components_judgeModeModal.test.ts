...
import { CURATED_5_STEPS, JUDGE_STEPS } from '../../apps/web/src/components/JudgeModeModal';
...
describe('Judge Mode Walkthrough Structure & Content', () => {
  ...
  it('verifies all 10 step IDs are sequential from 1 to 10', () => {
    const ids = JUDGE_STEPS.map((s: { id: number; title: string }) => s.id);
  ...
  it('includes navigation to the hand-curated safety fixture in approval gate step', () => {
    const approvalStep = JUDGE_STEPS.find((s: { id: number; title: string }) => s.title.includes('Approval Gate'));
    expect(approvalStep).toBeDefined();
    expect(approvalStep?.recommendedAction?.provenance).toBe('hand_curated_safety');
    expect(approvalStep?.technicalDetails.some((d: string) => d.includes('Hand-Curated Safety Fixture'))).toBe(true);
  });

  it('verifies that the reconciled accounting step covers the two zero-drift financial equations', () => {
    const accountingStep = JUDGE_STEPS.find((s: { id: number; title: string }) => s.title.includes('Reconciled Recovery Accounting'));
    expect(accountingStep).toBeDefined();
    expect(accountingStep?.technicalDetails.some((d: string) => d.includes('Equation 1'))).toBe(true);
    expect(accountingStep?.technicalDetails.some((d: string) => d.includes('Equation 2'))).toBe(true);
  });

  it('verifies the 5-stop curated pitch covers all high-leverage review stops', () => {
    const titles = CURATED_5_STEPS.map((s: { id: number; title: string }) => s.title);
