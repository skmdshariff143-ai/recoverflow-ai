...
| **8. Razorpay Test-Mode Integration** | Server-side execution adapter validating keys (`rzp_test_...`), enforcing timeouts, idempotency keys, live webhook HMAC-SHA256 verification, and simulator fallback. | `src/app/api/recovery/execute/route.ts`, `src/lib/adapters/recoveryAdapter.ts`, `src/app/api/recovery/webhook/route.ts` | Live Provenance selector, Execution receipt with transaction references. | `recoveryAdapter.test.ts` (12 tests) | Fully verified in `docs/LIVE_RAZORPAY_EVIDENCE.md`. Zero live keys permitted. | **PASS** |
