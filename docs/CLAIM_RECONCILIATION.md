...
| **3** | **Bounded Gemini AI Assistance** | `src/lib/ai/geminiClient.ts`, `/api/ai/diagnose`, `/api/ai/draft-message` | `geminiClient.test.ts` (7 tests) | Payment Drilldown AI Copilot drawer | `POST /api/ai/diagnose` | **PARTIAL** | Advisory integration with strict Zod validation. Deployed cloud environment operates via transparent `deterministic_fallback` when API key is unconfigured. |
