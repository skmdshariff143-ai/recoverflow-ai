...
import type { ExecutedItem, DataProvenanceSource } from '@recoverflow/core';
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-2.5 flex-1">
          ...
          <div className="flex items-center gap-1.5 text-xs text-slate-600">
            ...
            >
              ...
              {FAILURE_CATEGORIES.map((c: string) => (
        ...
        {/* Total records badge */}
        <div aria-live="polite" className="text-xs text-slate-500 flex items-center gap-2">
          <span>
        ...
        {paginatedItems.length === 0 ? (
          provenance === 'razorpay_test_mode' && totalCount === 0 ? (
            <div className="py-6 text-center bg-slate-900 text-white rounded-xl p-4 border border-indigo-500/40 shadow-sm space-y-3" data-testid="empty-queue-mobile">
              ...
              {onTriggerSampleWebhook && (
                <button
                  ...
                  className="mt-1 text-xs text-white font-bold bg-indigo-600 px-3 py-1.5 rounded-lg shadow-xs cursor-pointer"
                >
            ...
            <div className="py-8 text-center text-slate-400 space-y-2">
              <FilterX className="w-6 h-6 mx-auto text-slate-300" />
              <p className="text-xs font-medium">No failed payments match the selected filters</p>
              <button
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  {isRecovered ? (
                    ...
                    <span className="inline-flex items-center gap-1 font-semibold text-[10px] px-2 py-0.5 rounded-full bg-rose-100 text-rose-800 border border-rose-200">
                      <ShieldAlert className="w-3 h-3 text-rose-600" />
                      Stopped
                    </span>
                  ) : isPending ? (
                    <span className="inline-flex items-center gap-1 font-semibold text-[10px] px-2 py-0.5 rounded-full bg-amber-100 text-amber-800 border border-amber-200">
                      <AlertCircle className="w-3 h-3 text-amber-600" />
                      Pending Approval
                    </span>
                  ) : isDeferred ? (
                    <span className="inline-flex items-center gap-1 font-semibold text-[10px] px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 border border-slate-200">
                      <PauseCircle className="w-3 h-3 text-slate-400" />
                      Deferred
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-1 font-semibold text-[10px] px-2 py-0.5 rounded-full bg-blue-100 text-blue-800 border border-blue-200">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs border-y border-slate-100 py-2">
                  <div>
                    <span className="text-[10px] text-slate-600 block uppercase">Amount</span>
                  ...
                  <div className="text-right">
                    <span className="text-[10px] text-slate-600 block uppercase">Expected Value</span>
                ...
                <div className="flex items-center justify-between text-[11px] text-slate-500">
                  <span className="capitalize">{item.payment.failure_category.replace(/_/g, ' ')}</span>
              ...
              paginatedItems.map((item, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-2.5 px-3 text-center">
                      {isRecovered ? (
                        <span role="status" aria-label="Status: Recovered" className="inline-flex items-center gap-1 font-bold text-[11px] px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-300">
                          <CheckCircle2 className="w-3 h-3 text-emerald-600" />
                          Recovered
                        </span>
                      ) : isStopped ? (
                        <span
                          role="status"
                          aria-label={`Status: Stopped - ${item.stop_detail || 'Safety rule'}`}
                        ...
                        <span role="status" aria-label="Status: Pending Approval" className="inline-flex items-center gap-1 font-semibold text-[11px] px-2 py-0.5 rounded-full bg-amber-100 text-amber-800 border border-amber-200">
                          <AlertCircle className="w-3 h-3 text-amber-600" />
                          Pending Approval
                        </span>
                      ) : isDeferred ? (
                        <span role="status" aria-label="Status: Deferred" className="inline-flex items-center gap-1 font-semibold text-[11px] px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 border border-slate-200">
                          <PauseCircle className="w-3 h-3 text-slate-400" />
                          Deferred
                        </span>
                      ) : (
                        <span role="status" aria-label="Status: Retry Scheduled" className="inline-flex items-center gap-1 font-semibold text-[11px] px-2 py-0.5 rounded-full bg-blue-100 text-blue-800 border border-blue-200">
                          <Clock className="w-3 h-3 text-blue-600" />
                          Retry Scheduled
                        </span>
                      )}
